...
class AverageMetricsMeter():
    ...
    def add(self, metrics):
        ...
        for metric in metrics.values():
            ...
            RMSE, MEE, MAE, MMA = metric['rmse'], metric['mee'], metric['mae'], metric['mma']
            acceptable_homography = metric['homography_correct']
            ...
            if len(MMA) != 0:
                    ...
                    self.MMA5 += MMA[5]
                    self.count_mma+=1
            self.acceptable_homography+=acceptable_homography
            self.repeatability+=repeatability

            self.count+=1

    def value(self):
        return self.RMSE/self.count, self.MEE/self.count, self.MAE/self.count, \
               self.MMA1/self.count_mma, self.MMA3/self.count_mma,  self.MMA5/self.count_mma,  \
               self.acceptable_homography/self.count, self.repeatability/self.count
...
def compute_MMA_error(kp1, kp2, matches, real_H):
    lim = [1, 15]
    ...
    dist = (np.linalg.norm(true_warped_keypoints - warped_keypoints, axis=1))

    MMA = {thr: 0 for thr in rng}

    for thr in rng:
        MMA[thr] += np.mean(dist <= thr)
