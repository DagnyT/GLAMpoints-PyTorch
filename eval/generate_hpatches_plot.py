# d2-net/hpatches_sequences/HPatches-Sequences-Matching-Benchmark.ipynb
...
seq_to_skip = ['i_contruction', 'i_crownnight', 'i_dc', 'i_pencils', 'i_whitebuilding',
...
if __name__ == '__main__':
    ...
    methods = ['hesaff', 'hesaffnet', 'delf', 'delf-new', 'superpoint', 'lf-net', 'd2-net', 'd2-net-ms',
               'd2-net-trained', 'd2-net-trained-ms', 'glampoints_retina']
    names = ['Hes. Aff. + Root-SIFT', 'HAN + HN++', 'DELF', 'DELF New', 'SuperPoint', 'LF-Net', 'D2-Net', 'D2-Net MS',
             'D2-Net Trained', 'D2-Net Trained MS', 'GlamPoints_PSDataset+RootSift']
    colors = ['black', 'orange', 'red', 'red', 'blue', 'brown', 'purple', 'green', 'purple', 'green', 'orange']
    linestyles = ['-', '-', '-', '--', '-', '-', '-', '-', '--', '--', '--']
    top_k = None
    ...
    parser.add_argument('--path_to_hpatches_sequences', type=str,default='hpatches-sequences-release/', help='Path to HPatches dataset.')
    parser.add_argument('--path_to_cache_dir', type=str,default='cache/', help='cache dir.')

    print('Evaluate GlamPoints on HPatches dataset')

    opt = parser.parse_args()

    dataset_path =opt.path_to_hpatches_sequences

    if top_k is None:
        cache_dir = opt.path_to_cache_dir
    else:
        cache_dir = 'cache-top'
